...
def subdag_factory(parent_dag_id, subdag_dag_id, defaults_args, partner_settings):
    with DAG(f"{parent_dag_id}.{subdag_dag_id}", default_args=defaults_args) as dag:
        process_a(partner_settings["partner_name"], partner_settings["partner_name"])
        process_b(partner_settings["partner_name"], partner_settings["partner_name"])
        process_c(partner_settings["partner_name"], partner_settings["partner_name"])
    return dag
